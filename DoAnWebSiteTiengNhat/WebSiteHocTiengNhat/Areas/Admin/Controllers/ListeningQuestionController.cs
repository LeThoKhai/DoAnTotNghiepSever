...
namespace WebSiteHocTiengNhat.Areas.Admin.Controllers
{
    ...
    public class ListeningQuestionController : Controller
    {
        ...
        public ListeningQuestionController(ApplicationDbContext applicationDbContext)
        {
        ...
        {
            var list= _context.ListeningQuestions.ToList();
            return View(list);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(ListeningQuestion listeningQuestion, IFormFile? audioFile)
        {
            try
            {
                if (audioFile != null)
                {
                    ...
                    listeningQuestion.Link = audioPath;
                }
                else
                {
                    ModelState.AddModelError("", $"Thiếu file audio");
                    ViewBag.Level = new SelectList(new List<int> { 1, 2, 3, 4, 5 });
                    ViewBag.CorrectAnswer = new SelectList(new List<string> { "A", "B", "C", "D" });

                    return View(listeningQuestion);
                }
                await _context.AddAsync(listeningQuestion);
                _context.SaveChanges();
                return RedirectToAction("Index", "ListeningQuestion");
            }
            catch (Exception ex)
            {
        ...
        {
            var item= await _context.ListeningQuestions.FirstOrDefaultAsync(n=>n.Id==id);
            if (item == null)
            {
                return NotFound();
            }
            _context.ListeningQuestions.Remove(item);
            _context.SaveChanges();
            return RedirectToAction("Index", "ListeningQuestion");
        }
    }
